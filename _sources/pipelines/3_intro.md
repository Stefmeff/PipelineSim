...
The pipeline architectures we have dealt with so far, can all be classified as `synchronous circuits`. This means that they are run with a global `clock`, that dictates when and in what
...
provide an overview of the foundations and a couple of prominent examples for asynchronous pipelines and handshake protocols. The chapter is primarily based on the book `Introduction to Asynchronous Circuits Design` by Jens Sparso {cite}`Sparso20` and related papers.
